    ...
    if(mode_ == remote_wiimote)
    {
        ...
        else if (classic_connected == false)
        {
            ...
            { 
                ...
                if(state->buttons[MSG_BTN_UP])
                {
                    wii_twist.linear.y = -params.base_rot_speed * turbo_angular * params.d_pad_percent/100;
                }
                else if(state->buttons[MSG_BTN_DOWN])
                {
                    wii_twist.linear.y = params.base_rot_speed * turbo_angular * params.d_pad_percent/100;
...
void setparamsCallback(mesh_bot_control::mesh_bot_control_ParamsConfig &config, uint32_t level)
{
